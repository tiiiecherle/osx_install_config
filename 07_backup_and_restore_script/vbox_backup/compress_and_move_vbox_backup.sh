...
function archiving_tar_gz {
    ...
    pushd "$(dirname "$VBOXMACHINES")" 1> /dev/null; gtar -cpf - "$(basename "$VBOXMACHINES")" | pv -s "$PVSIZE" | pigz --best > "$VBOXTARGZFILE"; popd 1> /dev/null && echo '' && echo 'testing integrity of file(s)' && echo -n "$(basename "$VBOXTARGZFILE")"'... ' && gtar -tzf "$VBOXTARGZFILE" >/dev/null 2>&1 && echo file is OK || echo file is INVALID
