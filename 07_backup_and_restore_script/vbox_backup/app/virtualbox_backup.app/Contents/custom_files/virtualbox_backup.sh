...
if [[ "$(find "$VBOXTARGZSAVEDIR" -type f -name "*.tar.gz")" != "" ]]; then env_parallel --will-cite -j"$NUMBER_OF_MAX_JOBS_ROUNDED" --line-buffer "check_files_parallel {}" ::: "$(find "$VBOXTARGZSAVEDIR" -type f -name "*.tar.gz")"; fi
wait
