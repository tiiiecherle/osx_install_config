...
stop_if_running() {
    ...
    do
        ...
        if [[ $(pgrep "$COMMAND") != "" ]]
        then
            sudo killall -15 "$COMMAND" &>/dev/null
        else
            :
        fi
        NUM=0
        ...
        if [[ $(pgrep "$COMMAND") != "" ]]
        then
            sudo killall -9 "$COMMAND" &>/dev/null
        else
            :
        fi
        while [[ $(pgrep "$COMMAND") != "" ]]
...
### on demand directory scan app and notifier apps
#echo ''
echo "installing .app..."
. "$SCRIPT_DIR"/launchd_and_script/clamav_scan/dmg/clamav_scan/install_script/install.sh
wait
eval "$(typeset -f env_get_shell_specific_variables)" && env_get_shell_specific_variables
...
#echo ''
pids=()
sudo "$SCRIPT_INTERPRETER" -c "RUN_IN_TERMINAL_FROM_INSTALL_SCRIPT="yes" "$SCRIPT_INSTALL_PATH"/"$SCRIPT_INSTALL_NAME".sh" & pids+=($!)
#sudo "$SCRIPT_INTERPRETER" -c "RUN_IN_TERMINAL_FROM_INSTALL_SCRIPT="yes" "$SCRIPT_INSTALL_PATH"/"$SCRIPT_INSTALL_NAME".sh" &
# do not wait for script to finish as it is supposed to keep running while monitoring and scanning
#wait
while [[ $(pgrep "fswatch") == "" ]] || [[ $(pgrep "clamd") == "" ]]
do
    sleep 1
done
sleep 1
#echo ''
#sleep 3
pids+=$(ps aux | grep /"$SCRIPT_INSTALL_NAME".sh | grep -v grep | awk '{print $2;}')

echo "stopping script to test launchd service..."
stop_if_running
...
echo ''
echo "waiting for script from launchd to run correctly..."
#echo ''
sleep 3
WAIT_PIDS=()
